...
void HashTable::put(const string &s, int score) {
    int index = computeHash(s);
        list<WordEntry> currList = hashTable[index];
        for ( auto j : currList) { //for the index in the currlIst
            if (j.getWord() == s) {
                j.addNewAppearance(score);
            }
            else {
                //int appearance = 1;
                WordEntry *currWord = new WordEntry(s,score);
